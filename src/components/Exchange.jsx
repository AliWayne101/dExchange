...
axios.get(`${backendAddr}/exchangerate`)
.then(response => {
  ...
  const VerifyButton = () => {
    if (over_amount > 9.99) {
      if (userDetails.number.length > 10) {
        if (userDetails.number.startsWith('+923') || userDetails.number.startsWith('03')) {
    ...
    <section id="exchange" className={`${styles.paddingY} ${styles.flexCenter} flex-col relative`}>
      ...
      <div className="w-full flex justify-between items-center md:flex-row flex-col sm:mb-16 mb-6 relative z-[1]">
        <span className={styles.heading2}>
          ...
          {
            refLink !== "UNDEFINED" ? (
              <p className={`${styles.paragraph} mt-2`}>Reffered by: <b>{refLink}</b><br />Bonus: <b>+2 PKR per USD</b><br />Overall Bonus: <b>{(initialValue * CURRENCY.REFBONUS)} PKR</b></p>
            ) : ""
          }
        </span>
              ...
              errorMessage !== '' ? (
                ...
                <div className="w-full justify-center items-center text-center">
                  <h2 className={styles.heading2}>
                    ...
                    <span className={styles.paragraph}>An error has occured</span>
                </div>
                  <div className="w-full flex justify-center items-center">
                      <img src={confused} alt="error" className='rounded-[50%] h-[300px] w-[300px]' />
                  </div>
                  <div className='w-full flex justify-center items-center mt-5 text-secondary'>{errorMessage}</div>
                  <span onClick={resetError}><Button text="Retry!" styles='w-full mt-5'/></span>
                </>
              ) : (
                <>
                { 
                  ifSuccess === true ? (
                    <>
                      <div className="w-full flex justify-center items-center">
                        <img src={success} alt="success" className='rounded-[50%] h-[300px] w-[300px]'/>
                      </div>
                    ...
                    <>
                      ...
                      </label>
                      {
                        Show === true ? (
                          ...
                          <span onClick={VerifyButton}><Button text="Verify" styles="w-full" /></span>
                        )
                      }
                    </>
                  )
